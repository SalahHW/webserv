...
string utils::removeComments(const string &line) {
  ...
  if (commentPos != string::npos) return (line.substr(0, commentPos));
  return (line);
}

string utils::removeBraces(const string &line) {
  size_t openBracePos = line.find('{');
  if (openBracePos != string::npos) return (line.substr(0, openBracePos));
  return (line);
}
...
bool utils::isIntCompatible(const string &str) {
  ...
  if (errno == ERANGE || *endptr != '\0' || value > INT_MAX ||
      value < INT_MIN) {
    return false;
  }
  return true;

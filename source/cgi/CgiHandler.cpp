...
/*   Updated: 2025/01/29 15:29:47 by joakoeni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void CgiHandler::cgiExecution(const Request &request, int outputFd) {
  if (this->envVec.empty()) this->buildEnv(request);
  std::vector<std::string> env = this->getEnvVec();
  this->printEnv(env);
  std::cout << "[DEBUG] : BODY SIZE = " << request.getBody().size()
            << std::endl;

  const std::string scriptDirAss = "/home/joakoeni/webserv/var/www/cgi-bin";
  ...
  try {
    ...
    if (pid < 0) {
      ...
      if (dup2(pipefd[1], STDERR_FILENO) <
          0) {  // Optional: Redirect STDERR as well

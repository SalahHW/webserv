...
AutoIndexDirective::AutoIndexDirective(Block *currentContext,
...
AutoIndexDirective::AutoIndexDirective(const AutoIndexDirective &other)
    : Directive(other), autoIndex(other.autoIndex) {}

AutoIndexDirective &AutoIndexDirective::operator=(
    const AutoIndexDirective &other) {
  if (this != &other) {
...
bool AutoIndexDirective::validateSpecific() {
  if (getArguments()[0] == "on") {
    ...
    std::cerr << "Error: Directive 'autoindex' in " << getCurrentContext()
...
bool AutoIndexDirective::getAutoIndex() const { return autoIndex; }

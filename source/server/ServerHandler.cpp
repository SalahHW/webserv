...
void ServerHandler::handleClientWrite(int clientFd)
{
    ...
    if (client && client->hasDataToWrite()) {
        ...
        if (result == -1) {
            std::cerr << "Erreur lors de l'envoi des données au client " << clientFd << std::endl;
            closeClientConnection(clientFd);
        } else if (!client->hasDataToWrite()) {
            std::cout << "Toutes les données ont été envoyées au client " << clientFd << std::endl;
            if (client->shouldCloseConnection()) {
                std::cout << "Fermeture de la connexion pour le client " << clientFd << std::endl;
                closeClientConnection(clientFd);
            } else {
                std::cout << "Modification des événements epoll pour surveiller uniquement EPOLLIN pour le client " << clientFd << std::endl;
                modifyEpollEvent(clientFd, EPOLLIN | EPOLLONESHOT | EPOLLET);
            }
        }
    } else {
        std::cerr << "Client non trouvé ou aucune donnée à écrire pour le descripteur : " << clientFd << std::endl;
    }
}
...
void ServerHandler::handleClientRead(int clientFd)
{
    ...
    if (bytesRead > 0) {
        ...
        if (client) {
            ...
            std::cerr << "Client non trouvé pour le descripteur : " << clientFd << std::endl;
        }
    } else if (bytesRead == 0) {
...
void ServerHandler::handleNewConnection(Server& server)
{
    ...
    clientFd = accept(server.getListenFd(), (struct sockaddr*)&clientAddr, &clientLen);
    if (clientFd == -1) {
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            perror("accept");
        }
    ...
    if (setsockopt(clientFd, SOL_SOCKET, SO_SNDBUF, &sendBufSize, sizeof(sendBufSize)) == -1) {
...
void ServerHandler::closeClientConnection(int clientFd)
{
    ...
    if (epoll_ctl(epollFd, EPOLL_CTL_DEL, clientFd, NULL) == -1) {
        ...
        std::cout << "Descripteur " << clientFd << " supprimé de epoll." << std::endl;
    }
    ...
    for (std::map<int, Server>::iterator serverIt = serversList.begin(); serverIt != serversList.end(); ++serverIt) {
        ...
        if (clientIt != clients.end()) {
            ...
            std::cout << "Client " << clientFd << " supprimé de la liste des clients." << std::endl;
            break;
        }
    }
}
...
void ServerHandler::startToListen()
{
    ...
    while (1) {
        ...
        for (int i = 0; i < this->nbEvents; i++) {
            ...
            std::map<int, Server>::iterator serverIt = this->serversList.find(currentFd);
            if (serverIt != this->serversList.end()) {
                std::cout << "Événement sur le descripteur d'écoute : " << currentFd << std::endl;
                handleNewConnection(serverIt->second);
            } else {
                if (eventFlags & EPOLLIN) {
                    std::cout << "Événement EPOLLIN détecté pour le descripteur : " << currentFd << std::endl;
                    handleClientRead(currentFd);
                }
                if (eventFlags & EPOLLOUT) {
                    std::cout << "Événement EPOLLOUT détecté pour le descripteur : " << currentFd << std::endl;
                    handleClientWrite(currentFd);
                }
                if (eventFlags & (EPOLLHUP | EPOLLERR)) {
                    std::cerr << "Erreur ou fermeture sur le descripteur : " << currentFd << std::endl;

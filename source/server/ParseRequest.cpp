...
void ParseRequest::findAndParseRequestLine(std::string::size_type& headersStartPos)
{
    ...
    stream >> this->requestParsed.method >> this->requestParsed.uri >> this->requestParsed.version;

    headersStartPos = requestLineEnd + 2;
}

void ParseRequest::findAndParseHeaders(std::string::size_type headersStartPos, std::string::size_type& bodyStartPos)
{
...
{
    if (this->requestParsed.method == "POST" || this->requestParsed.method == "PUT") {
        ...
        if (it != this->requestParsed.headers.end()) {
            ...
            if (contentLength > 0 && this->request.size() >= bodyStartPos + contentLength) {
...
void ParseRequest::showHttpRequest() const
{
    ...
    for (std::map<std::string, std::string>::const_iterator it = requestParsed.headers.begin(); it != requestParsed.headers.end(); ++it) {

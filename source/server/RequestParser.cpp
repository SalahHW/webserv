...
RequestParser::RequestParser(const RequestParser& src)
    : request(src.request)
    , requestToFill(src.requestToFill)
{
}
...
void RequestParser::parseMultipartBody(const std::string& bodyData)
{
  ...
  if (startPos < bodyData.size() && bodyData.compare(startPos, 2, "\r\n") == 0)
  {
    startPos += 2;
  }
...
void RequestParser::parseSinglePart(const std::string& part)
{
  ...
  if (filenamePos != std::string::npos)
  {
    ...
    {
      std::string filename = dispLine.substr(filenamePos, filenameEnd - filenamePos);
  ...
  size_t startSearch = (ctypeLineEnd != std::string::npos) ? ctypeLineEnd : dispLineEnd;
  ...
  if (fileData.size() >= 2 && fileData.compare(fileData.size() - 2, 2, "\r\n") == 0)
  {

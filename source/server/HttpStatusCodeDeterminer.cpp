...
void HttpStatusCodeDeterminer::determineStatusCode(RequestParsed& requestParsed)
{
    // 1. Check for basic request validity
    if (requestParsed.method.empty() || requestParsed.uri.empty() || requestParsed.version.empty()) {
    ...
    if (requestParsed.version != "HTTP/1.1") {
        requestParsed.statusCode = HTTP_VERSION_NOT_SUPPORTED; // 505 HTTP Version Not Supported
        return;
    }

    // 3. Check if the method is implemented
    if (requestParsed.method != "GET" && requestParsed.method != "POST" && requestParsed.method != "DELETE") {
    ...
    if ((requestParsed.method == "POST" || requestParsed.method == "PUT") && requestParsed.headers.find("Content-Length") == requestParsed.headers.end()) {
        requestParsed.statusCode = LENGTH_REQUIRED; // 411 Length Required
        return;
    }

    // 9. Check for payload size
    std::map<std::string, std::string>::const_iterator it = requestParsed.headers.find("Content-Length");
    if (it != requestParsed.headers.end()) {
        size_t contentLength = std::strtoul(it->second.c_str(), NULL, 10);
        if (contentLength > static_cast<size_t>(matchingLocation.getClientMaxBodySize())) {
    ...
    if ((requestParsed.method == "POST" || requestParsed.method == "PUT") && requestParsed.headers.find("Content-Type") != requestParsed.headers.end()) {
        std::string contentType = requestParsed.headers.at("Content-Type");
        if (!isSupportedMediaType(contentType)) {
            requestParsed.statusCode = UNSUPPORTED_MEDIA_TYPE; // 415 Unsupported Media Type
            return;
        }
    }
    ...
    if (requestParsed.headers.find("Range") != requestParsed.headers.end()) {
        ...
        if (stat(filePath.c_str(), &statbuf) == 0) {
            ...
            if (!isValidRange(requestParsed, fileSize)) {
                requestParsed.statusCode = RANGE_NOT_SATISFIABLE; // 416 Range Not Satisfiable
                return;
            }
        } else {
            requestParsed.statusCode = INTERNAL_SERVER_ERROR; // 500 Internal Server Error
            return;
        }
    }
...
bool HttpStatusCodeDeterminer::findMatchingLocation(const std::string& uri, Location& matchingLocation)
{
...
bool HttpStatusCodeDeterminer::isMethodAllowed(const Location& location, const std::string& method)
{
    if (method == "GET")
        return location.getGetAccepted();
    if (method == "POST")
        return location.getPostAccepted();
    if (method == "DELETE")
        return location.getDeleteAccepted();
    return false;
}
...
bool HttpStatusCodeDeterminer::hasAccessPermissions(const std::string& filePath)
{
    return (access(filePath.c_str(), R_OK) == 0);
}

bool HttpStatusCodeDeterminer::isSupportedMediaType(const std::string& contentType)
{
...
bool HttpStatusCodeDeterminer::isValidRange(const RequestParsed& requestParsed, size_t fileSize)
{
    // Vérifie si l'en-tête "Range" existe
    std::map<std::string, std::string>::const_iterator it = requestParsed.headers.find("Range");
    if (it == requestParsed.headers.end()) {
...
std::string HttpStatusCodeDeterminer::determineFilePath(const Location& location, const std::string& uri)
{
